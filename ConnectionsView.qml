...
// The connection list, the edit form and the delete confirmation: everything the
// bar popup shows, as one item that any surface can host.
//
// Two surfaces do. Panel.qml puts it inside the bar's KeyboardPanel, once per
// monitor, and ConnectionsWindow.qml puts it inside a FloatingWindow, once per
// shell. Each host gets its own copy of the state that genuinely belongs to a
// surface — cursor, scroll position, form fields — so a form half-filled in the
// window is not also half-filled in the popup. Session state, the connection
// list and every file write stay in Service.qml, reached through `svc`.
//
// Keys are dispatched by the PanelKeyCatcher inside; hosts hand it focus through
// `keyCatcher`. Whatever a key means only to the host — Esc when nothing is left
// to unwind, Tab, "open the window" — is raised as a signal rather than acted on
// here, because the popup and the window answer them differently.
Item {
  id: root
  ...
  // True while the host is showing this view. Flipping it resets the transient
  // state below, the way closing the popup always has.
  property bool surfaceOpen: false

  // The window host sets this. It hides the "open in a window" control, which
  // would be a strange thing to offer from inside the window.
  property bool inWindow: false

  // The item a host should give keyboard focus once it is on screen.
  readonly property alias keyCatcher: keyCatcher

  // How tall the content wants to be; the popup fits its card to this.
  readonly property real contentHeight: column.implicitHeight

  // Esc with nothing left to unwind, Tab, and the window control. The popup
  // closes / switches panels / opens the window; the window hides itself,
  // ignores Tab and never sees the third.
  ...
  readonly property string glyphWindow: "\u{F03CB}"    // nf-md-open-in-app
  ...
  readonly property bool anyActive: summary.state === "connected" || summary.state === "connecting"
  ...
  function onTextKey(t) {
    ...
    if (key === "w") { if (!root.inWindow) root.openWindowRequested(); return }
  ...
  // Reset transient view state whenever the surface closes, so reopening never
    ...
    Flickable {
      ...
      Column {
        ...
        PanelHero {
          ...
          // The same view, in a window that can be resized and left open. Only
          // from the popup: the window has no use for a button that opens it.
          trailingControl: root.inWindow ? null : windowButton
        }
  ...
  Component {
    ...
    PanelActionButton {
      visible: root.view === "list"
      iconText: root.glyphWindow
      tooltipText: "Open in a window  (w)"
      foreground: root.foreground
      fontFamily: root.fontFamily
